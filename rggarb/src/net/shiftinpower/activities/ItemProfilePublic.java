...
public class ItemProfilePublic extends RggarbSlidingMenu implements OnGetItemDataListener {
	...
	public void onGetItemDataSuccess(ItemExtended itemParameters) {
		...
		int[] initialImageViews = { R.id.iItemProfileImageSlotTwo, R.id.iItemProfileImageSlotThree, R.id.iItemProfileImageSlotFour, R.id.iItemProfileImageSlotFive };
		...
		for (UserBasic itemUser : itemUsers) {
			if (a < initialUserAvatarViewsCount) {
				...
				fetchableImageView.setVisibility(View.VISIBLE);
				fetchableImageView.setImage(C.API.WEB_ADDRESS + C.API.IMAGES_USERS_FOLDER_THUMBNAIL + itemUser.getUserAvatar(), R.drawable.images_default_avatar_male);
				a++;
			}
		}
		...
		if (friendsAvatarsUrls != null) {
			...
			for (int b = 0, c = 0; b < initialFriendAvatarViews.size() &&  c < friendsAvatarsUrls.size(); c++, b++) {
				...
				FetchableImageView fetchableImageView = (FetchableImageView) findViewById(initialFriendAvatarViews.get(b));
				fetchableImageView.setVisibility(View.VISIBLE);
				fetchableImageView.setImage(C.API.WEB_ADDRESS + C.API.IMAGES_USERS_FOLDER_THUMBNAIL + friendsAvatarsUrls.get(b), R.drawable.images_default_avatar_male);
		...
		for (Place itemPlace : itemPlaces) {
			if (c < initialPlaceAvatarViewsCount) {
				FetchableImageView fetchableImageView = (FetchableImageView) findViewById(initialPlaceAvatarViews.get(c));
				fetchableImageView.setVisibility(View.VISIBLE);
				fetchableImageView.setImage(C.API.WEB_ADDRESS + C.API.IMAGES_PLACES_FOLDER_THUMBNAIL + itemPlace.getPlaceAvatar(), R.drawable.images_default_shop);
				c++;
			}
		}
